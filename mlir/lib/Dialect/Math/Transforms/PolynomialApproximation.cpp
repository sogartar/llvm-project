...
namespace {
template <typename CoefficientIt>
Value makePolynomialCalculation(ImplicitLocOpBuilder &builder,
                                CoefficientIt coeffsBegin,
                                CoefficientIt coeffsEnd, Value x) {
  auto coeffsCount = coeffsEnd - coeffsBegin;
  ...
  Value res = builder.create<math::FmaOp>(x, coeffsEnd[-1], coeffsEnd[-2]);
  for (auto coeffIt = coeffsEnd - 3; coeffIt >= coeffsBegin; --coeffIt) {
    res = builder.create<math::FmaOp>(x, res, *coeffIt);
  }
  return res;
}
} // namespace
...
ErfApproximation::matchAndRewrite(math::ErfOp op,
  ...
  Value pPoly =
      makePolynomialCalculation(builder, std::begin(p), std::end(p), x);
  Value qPoly =
      makePolynomialCalculation(builder, std::begin(q), std::end(q), x);
